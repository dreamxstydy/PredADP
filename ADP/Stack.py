...
Randon_seed = 10
njobs = 8
Path('./Models/Train/').mkdir(exist_ok = True,parents = True)
...
def base_clf(clf,X_train,y_train,model_name,n_folds=5):
    ...
    joblib.dump(clf,f'./Models/Train/{model_name}.model')
    return base_train[:,-1]
...
def Stackfeature(file):
    final_Features = []
    feature_Name = ['AAC', 'BPNC', 'CTD', 'DPC']
    y = file[0]
    y = np.array([1 if i < 407 else 0 for i in range(814)])
    ...
    file_path = ('ADP/Train/Features.csv')
